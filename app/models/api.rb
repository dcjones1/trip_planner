...
class Api
  ...
  def maps_call(origin, destination, url)
...
# Airports within 200 miles of Washington DC:
    ...
    # url = "https://maps.googleapis.com/maps/api/directions/json?origin=#{origin}&destination=#{destination}&key=#{GOOG_API_KEY}"
...
def get_distance_hash(address)
  ...
  hash = Hash[airports.map do |airport|
    ...
    destination = airport
    mode = Api.new
    hash = mode.maps_call(origin, destination, "https://maps.googleapis.com/maps/api/directions/json?origin=#{origin}&destination=#{destination}&key=#{GOOG_API_KEY}")
    hash["routes"][0]["legs"][0]["distance"]["text"].split(" ")[0].to_f
  end.zip(airports)]
   hash
end

def find_airports_within_radius(radius, hash)
  radius = radius.to_f
  airport_options = []
